...
uint16_t modbus_update(uint16_t *holdingRegs){
  ...
  uint16_t crc16;
	...
	while (!((((UART_T *) ((( uint32_t)0x40000000) + 0x50000)))->FSR & (1ul << 14))){
		if (overflow)
			((((UART_T *) ((( uint32_t)0x40000000) + 0x50000)))->RBR);
		else{
			if (bufferIdx == 128)
				overflow = 1;	
			
			
			UART_Read(((UART_T *) ((( uint32_t)0x40000000) + 0x50000)),frame+bufferIdx,1);
			bufferIdx++;
		}
		CLK_SysTickDelay(T1_5); 
	}
    ...
    if (rcv_id == slaveID || broadcastFlag) { 
      rcv_crc = ((frame[bufferIdx - 2] << 8) | frame[bufferIdx - 1]); 
			
      if (calculateCRC(bufferIdx - 2) == rcv_crc) { 
				function = frame[1];                          
...
void sendPacket(uint8_t bufferSize)
{
	...
	for ( i = 0; i < bufferSize; i++)
		UART_Write(((UART_T *) ((( uint32_t)0x40000000) + 0x50000)), frame+i,1);
...
void UART0_Init(uint32_t baudrate){
    ...
    SYS_ResetModule(((0x4<<24) | 16 ));
	 ...
	 UART_SetLine_Config(((UART_T *) ((( uint32_t)0x40000000) + 0x50000)), baudrate,(3),(0x3 << 3),(0x0 << 2));
}
